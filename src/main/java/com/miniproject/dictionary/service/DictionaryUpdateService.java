...
public class DictionaryUpdateService implements CommandProcess {
	...
	public String requestProcess(HttpServletRequest request, HttpServletResponse response)
		...
		if (contentType != null && contentType.toLowerCase().startsWith("multipart/")) {
			...
			for (Part part : parts) {
				...
				if (partHeader.contains("filename=")) {
					...
					switch (paramName) {
					case "animalId" -> animal.setId(Integer.parseInt(paramValue));
		...
		if (type.equals("dog")) {
			...
			dao.updateDictionaryDog(dog);
		}

		else if (type.equals("cat")) {
			cat.getAnimal(animal);
			dao.updateDictionaryCat(cat);
		}

		return "r:dictionaryList";
	}
