					...
					<tbody>
						...
						<tr>
							...
							<td><c:if test="${empty board.file1}">
									...
									<a
										href="${pageContext.request.contextPath}/free/fileDownload?fileName=${board.file1}">파일
										다운로드</a>
								</c:if></td>
							<th>조회수</th>
		...
		<div class="row my-3">
			<div class="col text-center">
				<c:if test="${board.writer == sessionScope.id}">
					<input type="button" class="btn btn-primary" id="detailUpdate"
						value="수정하기" data-board-no="${board.no}">
				</c:if>

				<c:if
					test="${(board.writer == sessionScope.id) || sessionScope.isAdmin}">
					<input type="button" class="btn btn-danger ms-2 me-2"
						id="detailDelete" value="삭제하기" data-board-no="${board.no}">
				</c:if>
			...
			<div class="col border p-3">
				<div id="recommend" class="text-end"
					...
					<span id="commend" class="btnCommend text-primary"
						style="cursor: pointer;text-align: left;display: flex;align-items: center;"> <img
						src="${pageContext.request.contextPath}/images/recommend.png"
						alt="추천하기"> 추천 <span class="recommend">(${board.recommend})</span>
					</span> | <span id="thank" class="btnCommend text-primary"
						style="cursor: pointer;text-align: left;display: flex;align-items: center;"> <img
						src="${pageContext.request.contextPath}/images/smile.png" alt="땡큐">
						땡큐 <span class="recommend">(${board.thank})</span>
					</span> | <span id="replyWrite" class="text-primary"
						style="cursor: pointer;"> <i class="bi bi-file-text-fill"
						style="color: cornflowerblue;"></i> 댓글쓰기
					</span>
				</div>
			</div>
		</div>
								...
								<div class="row bg-light p-2">
									...
									<div class="col-8 text-end p-2">
										<span class="me-3"> <fmt:formatDate
												value="${reply.regDate}" pattern="yyyy-MM-dd HH:mm:dd" />
										</span>
										...
										<button class="reportReply btn btn-outline-danger btn-sm"
											data-reply-no="${reply.no}">
			...
			<div class="col">
				<form name="replyWriteForm" id="replyWriteForm">
					<input type="hidden" name="boardNo" value="${board.no}"> <input
						type="hidden" name="replyWriter" value="${sessionScope.id}">
