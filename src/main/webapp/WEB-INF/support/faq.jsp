					...
					<div class="accordion-item">
						<h2 class="accordion-header" id="heading${faq.faqNo}">
							<button class="accordion-button collapsed" type="button"
								data-bs-toggle="collapse" data-bs-target="#collapse${faq.faqNo}"
								aria-expanded="false" aria-controls="collapse${faq.faqNo}">

								<span>${faq.faqTitle }</span> <span class="flex-grow-1"></span>
								<c:if test="${isAdmin == true }">
									<%-- <a> 태그를 <span>으로 변경하고, data-href 속성에 URL 저장 --%>
									<span
										data-href="${pageContext.request.contextPath}/support/faqUpdateForm?no=${faq.faqNo}"
										class="btn btn-sm btn-outline-secondary me-2 edit-faq-action z-3"
										role="button" tabindex="0">수정</span>
									<span
										data-href="${pageContext.request.contextPath}/support/faqDelete?no=${faq.faqNo}"
										class="btn btn-sm btn-outline-danger me-2 delete-faq-action z-3"
										role="button" tabindex="0">삭제</span>
								</c:if>
							</button>
						</h2>
						<div id="collapse${faq.faqNo}" class="accordion-collapse collapse"
							aria-labelledby="heading${faq.faqNo}"
							data-bs-parent="#accordionExample">
							<div class="accordion-body">${faq.faqContent }</div>
						</div>
					</div>
				</c:forEach>
			</div>
				...
				<nav aria-label="Page navigation example">
					<ul class="pagination justify-content-center my-5">
						...
						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							...
							<c:if test="${i != currentPage}">
								<li class="page-item"><a class="page-link"
									href="${pageContext.request.contextPath}/support/faq?pageNum=${i}&type=${type}&keyword=${keyword}">${i}</a>
								</li>
							</c:if>
						</c:forEach>
				...
				<nav aria-label="Page navigation example">
					<ul class="pagination justify-content-center my-3">
						...
						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							...
							<c:if test="${i != currentPage}">
								<li class="page-item"><a class="page-link"
									href="${pageContext.request.contextPath}/support/faq?pageNum=${i}">${i}</a>
								</li>
							</c:if>
						</c:forEach>
						<c:if test="${endPage < pageCount}">
...
<script>
	document.addEventListener('DOMContentLoaded', function() {
		...
		document.querySelectorAll('.edit-faq-action').forEach(
		...
		document.querySelectorAll('.delete-faq-action').forEach(
				function(spanElement) {
					spanElement.addEventListener('click', function(event) {
						// 1. 이벤트 버블링 중단 (매우 중요!)
						event.stopPropagation();
