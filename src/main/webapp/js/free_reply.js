$(function() { // DOM이 준비되면
	...
	$("#replyList").on("click", ".reportReply", function(e) {
		var isReport = confirm("이 댓글을 신고할까요?");

		if (isReport) {
			
			var replyNo = $(this).data("replyNo");
			var replyContent = $("#replyList").find('pre[data-reply-no="' + replyNo + '"]').text();
			var freeTitle = $("#freeTitle").text();
			
			var data = { replyContent: replyContent, title: freeTitle};

			$.ajax({
				url: "replyReport.ajax",
				type: "post",
				data: data,
				dataType: "json",
				success: function(resData) {
