...
$(function() { // DOM이 준비되면
	...
	$(document).on("click",".deleteReply", function(e) {
		...
		let params = "replyNo=" + no +"&bbsNo=" + bbsNo; 
		console.log(params);
		...
		$.ajax({
			...
			"success": function(resData) {
				...
				// resData => 댓글 리스트 = [{no:20, bbsNo: 200, replyTitle: "" ,...}, {}, ...]
				console.log(resData);
	...
	$(document).on("submit","#replyUpdateForm", function(e) {
		// 폼이 서브밋 될 때 제일 먼저 - 댓글이 5자 이상 입력되었는지 체크 - 유효성 검사
		...
		// 데이터를 준비해서 서버로 보내야 함	
		// bbsNo=10&replyWriter=admin&replyContent=dfsfdsfds&no=3
